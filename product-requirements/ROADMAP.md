...
6-week plan per spec Section 17, plus post-spec amendments (spec Section 25).

**Geography**: Ireland-first in V1. The single knob to retarget another country is `default_country` in [`config/profile.yml`](../config/profile.example.yml). See spec Section 25.1.
...
| 2.5 | Job discovery via Adzuna + Reed official APIs (scope expansion Section 25.2) | ✅ done |
...
| 2.10 | Session auth + encrypted settings store + login page (spec Section 25.4) | ✅ done |
| 2.11 | Settings UI page (API keys, model overrides, budgets, change password Section 25.5) | ✅ done |
...
| 3 | Agentic research loop + MCP server + Tavily | **next** |
...
These items came from user direction during build and exist in addition to the spec Section 17 weekly plan:
...
- **Job discovery via partner APIs** (Adzuna, Reed) — spec Section 3.2 originally banned all scraping; user-authorised override in Section 25.2 covers official-API sources only. LinkedIn/Indeed remain extension-only.
...
- **Usage page** at `/ui/usage.html` — month-to-date spend, by model, by node, recent calls. Realises the spec Section 9 observability gap.
- **Session auth + AES-GCM settings store** (spec Section 25.4) — added because the Settings UI exposes editable API keys. Single-user only; SSO / RBAC / MFA explicitly out of scope.
...
Spec Section 23 self-check answers:
...
See spec Section 17 + Section 25. If a week slips, **eval harness and MCP server remain non-negotiable** (spec Section 17 closing note). Cut UI polish or research-loop sophistication first.
